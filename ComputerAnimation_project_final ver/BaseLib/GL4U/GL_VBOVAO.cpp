...
GL_VBOGroup::SetByMesh(const mg::Mesh &mesh)
{
	...
	unsigned int *index_data = new unsigned int[m.num_faces()*3];
...
static bool g_f_update_normals;
static bool g_f_update_uvs;
static bool g_f_update_colors;
static bool g_f_update_vertices;

void 
GL_VBOGroup::BeginUpdate()
{
	g_tmp_normals.clear();
	g_tmp_uvs.clear();
	g_tmp_colors.clear();
	...
	g_f_update_normals = false;
	g_f_update_uvs = false;
	g_f_update_colors = false;
	g_f_update_vertices = false;
}

void 
GL_VBOGroup::glUpdateNormal(cml::vector3f n)
{
	g_last_normal = n;
	g_f_update_normals = true;
}
void 
GL_VBOGroup::glUpdateUV(cml::vector2f uv)
{
	g_last_uv = uv;
	g_f_update_uvs = true;
}
...
void 
GL_VBOGroup::glUpdateColor(cml::vector4f c)
{
	g_last_color = c;
	g_f_update_colors = true;
}

void 
GL_VBOGroup::glUpdateVertex(cml::vector3f v)
{
	g_tmp_normals.push_back(g_last_normal);
	g_tmp_uvs.push_back(g_last_uv);
	g_tmp_colors.push_back(g_last_color);
	g_tmp_vertices.push_back(v);
	g_f_update_vertices = true;
}

void
GL_VBOGroup::EndUpdate()
{
	if ( (int)g_tmp_vertices.size() != g_num_vertices ) return;

	if ( g_f_update_vertices )
		UpdateVBO(VERTEX_VBO, GL_FLOAT, 3, g_tmp_vertices.data());
	if ( g_f_update_normals )
		UpdateVBO(NORMAL_VBO, GL_FLOAT, 3, g_tmp_normals.data());
	if ( g_f_update_colors )
		UpdateVBO(COLOR_VBO, GL_FLOAT, 4, g_tmp_colors.data());
	if ( g_f_update_uvs )
		UpdateVBO(TEX_COORD_VBO, GL_FLOAT, 2, g_tmp_uvs.data());
}
...
GL_VAO::Draw() const
{
	...
	//	{
	//		int i = *iter;
	//		cml::matrix44f_c m_and_offset = GetBoneMatrix(i) * cml::inverse( GetBoneOffsetMatrix(i) );
	//		shader_->SetBoneMatrix(i, m_and_offset.data() ); 
	//	}
