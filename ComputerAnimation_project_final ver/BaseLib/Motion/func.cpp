...
std::string ml::write_motions( const std::vector<ml::Motion> &motions, const std::string &name )
{
  std::string file_name;
  file_name = std::string("./data/save/") + name + ".mot";

  //cout << "save file name : " << file_name << endl;
  std::ofstream file(file_name.c_str(), std::ios::out);

  //motions
  file << motions.size() << std::endl;
  for (auto it_m = motions.begin(); it_m != motions.end(); ++it_m) {
	...
	for (auto it_p = it_m->m_postures.begin(); it_p != it_m->m_postures.end(); ++it_p) {
	  ...
	  file << p.time << ' ' << p.leftFootTouched << ' ' << p.rightFootTouched << ' ' << p.type_ << ' ' << p.object << std::endl;
...
void ml::read_motions( std::vector<ml::Motion> &motions, const std::string &name ) 
{
  ml::Motion base_m;
  base_m.LoadAMC_with_contactInfo("./data/test_motion.amc", "./data/wd2.asf", true, 0.027);
  motions.clear();

  std::ifstream file("./data/save/" + name + ".mot", std::ios::in);
  if ( file.fail() ) {
	std::cout << "no mot file: " << name << std::endl;
	return;
  }
