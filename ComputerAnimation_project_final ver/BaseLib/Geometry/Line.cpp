...
{
	if ( cml::length(cml::cross(cml::normalize(line_B.dir_), cml::normalize(this->dir_))) < cml::epsilond().placeholder() ) 
		return true;
...
Line::BridgeToLine(const Line &line_B) const
{
	...
	const cml::vector3d &pA = line_A.point_;
	const cml::vector3d &pB = line_B.point_;
	...
	cml::matrix33d_c A;
	cml::vector3d x, b;
	A.set_basis_element(0, 0, vA[0]);
	A.set_basis_element(0, 1, -1*vB[0]);
	A.set_basis_element(0, 2, -1*vC[0]);

	A.set_basis_element(1, 0, vA[1]);
	A.set_basis_element(1, 1, -1*vB[1]);
	A.set_basis_element(1, 2, -1*vC[1]);

	A.set_basis_element(2, 0, vA[2]);
	A.set_basis_element(2, 1, -1*vB[2]);
	A.set_basis_element(2, 2, -1*vC[2]);
