...
{
	local_translation_ = Rotate(r, local_translation_);
...
{
	translation_ = Rotate(r, translation_);
...
PrimitiveCylinder::BuildMesh(Mesh &out_mesh)
{
	...
	if ( direction_ == 0 )
		out_mesh.RotateVertices(QuaterRotationAxisAngle(cml::z_axis, -1.0*M_PI));
	else if ( direction_ == 2 )
		out_mesh.RotateVertices(QuaterRotationAxisAngle(cml::x_axis, M_PI));
...
PrimitiveCylinder::BuildSolidObject(SolidObject &out_obj)
{
	...
	if ( direction_ == 0 )
		out_obj.editable_mesh()->RotateVertices(QuaterRotationAxisAngle(cml::z_axis,-1.0*M_PI));
	else if ( direction_ == 2 )
		out_obj.editable_mesh()->RotateVertices(QuaterRotationAxisAngle(cml::x_axis, M_PI));
...
PrimitiveCapsule::GetGlobalCylinderEndPoints(cml::vector3d &p0, cml::vector3d &p1) const
{
	...
	p0 = Rotate(local_rotation_, p0);
	p0 += local_translation_;
	p0 = Rotate(rotation_, p0);
	p0 += translation_;

	p1 = Rotate(local_rotation_, p1);
	p1 += local_translation_;
	p1 = Rotate(rotation_, p1);
	p1 += translation_;
}
...
PrimitiveCapsule::SetGlobalCylinderEndPoints(cml::vector3d p0, cml::vector3d p1) 
{
	...
	RotateGlobally(QuaterRotationAxisAngle(cml::normalize(cml::cross(cml::y_axis,v)), atan2( cml::length(cml::cross(cml::y_axis, v)), cml::y_axis%v )));
