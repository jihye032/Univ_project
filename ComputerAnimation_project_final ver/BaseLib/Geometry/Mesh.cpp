...
Mesh::CreateCylinder(cml::vector3d bottom_center, cml::vector3d top_center, double radius, int slices, int stacks)
{
	...
	if ( cml::length(n-cml::y_axis) <= 0.0001 )
	{
	...
	{
		rotation_axis = cml::cross(cml::y_axis, n);
		rotation_angle = atan2( cml::length(rotation_axis), cml::y_axis%n );
	...
	cml::quaterniond q;
	RotateVertices(QuaterRotationAxisAngle(rotation_axis, rotation_angle));
...
Mesh::CreateCylinder(double height, double radius, int slices, int stacks)
{
	...
	{
		cml::vector3d tmp_p = Rotate( QuaterRotationEuler(cml::vector3d(0, i*(2*M_PI)/slices, 0))
	...
	// set normals
	{
		int n_count = 0;
		// top, index 0
		normal(n_count, cml::y_axis);			
		n_count++;

		// bottom, index 1
		normal(n_count, -1*cml::y_axis);		
		n_count++;
...
Mesh::CreateCapsule(double cylinder_height, double radius)
{
	...
	bottom_hemi.RotateVertices(QuaterRotationEuler(cml::vector3d(0, 0, M_PI)));
...
Mesh::CalculFaceNormal(int face_id) const
{
	...
	return normalize(cml::cross(side0,side1));
}
